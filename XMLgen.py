...
xmlFile = open("Z:\partners\%s\\request_%s_sf%s\ZI_%s_%s.xml" % (newfolder, currentDate, newfolder2, client_name, job_name), "w")	
...
def personMatcher():
	...
	xmlFile.write("        </property>\n")
	xmlFile.write("    </bean>\n")
	xmlFile.write("  <bean id=\"dataservices.determinePastOrPresentMatches\" class=\"java.lang.Boolean\">\n")
	xmlFile.write("    <constructor-arg value=\"true\" />\n")
	xmlFile.write("  </bean>")
	xmlFile.write("</beans>")
...
def queryList():
	...
	numQueries = int(queries)
	for i in range(numQueries):
		#constructs one query bean.
		xmlFile.write("        <bean class=\"com.zoominfo.component.dsConfig.QueryConfig\">\n")
		queryName = raw_input("Please enter a name for query number %d: " %(i+1))
		xmlFile.write("            <property name=\"name\" value=\"%s\"/>\n" % queryName)
		queryUrl = raw_input("Please paste the URL for query number %d: " %(i+1))
		xmlFile.write("            <property name=\"query\" value=\"%s\"/>\n" % queryUrl)
		xmlFile.write("        </bean>\n")
	xmlFile.write("    </util:list>\n")

def companyMatcher():
	companyInputFile = raw_input("What is the name of the input CSV file (without extension)? ")
	xmlFile.write("    <bean id=\"companyMatcher\" class=\"com.zoominfo.component.dsConfig.CompanyMatcherConfig\">\n")
	xmlFile.write("        <property name=\"inputDataFiles\">\n")
	xmlFile.write("            <list>\n")
	xmlFile.write("                <bean id=\"companyMatchDataFile\" class=\"com.zoominfo.component.dsConfig.InputDataFileConfig\">\n")
	xmlFile.write("                    <property name=\"filePath\" value=\"c:\partners\%s\\request_%s_sf%s\%s.csv\"/> <!-- path to input company file name -->\n" % (newfolder, currentDate, newfolder2, companyInputFile)) 
	xmlFile.write("                    <property name=\"hasHeader\" value=\"true\"/>\n")
	xmlFile.write("                </bean>\n")
	xmlFile.write("            </list>\n")
	xmlFile.write("        </property>\n")
